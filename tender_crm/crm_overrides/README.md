...
| `territory_geo.py` | CRM Territory ↔ Country ↔ Currency/Timezone. `validate` on CRM Territory rejects a country listed twice and fills blank row currencies; `validate` on CRM Organization defaults `currency` (territory row, else country) and `tsi_timezone` (country's first zone) when blank and rejects a timezone outside the country's zones. The interactive dropdowns are the "TSI Client Territory Geo" form script (`territory_geo_schema.py`), fed by `api/territory_geo.py`. |
